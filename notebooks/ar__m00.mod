TITLE Mod file for component: Component(id=ar__m00 type=ionChannelHH)
...
NEURON {
    ...
    RANGE gion
    RANGE i__ar__m00 : a copy of the variable for current which makes it easier to access from outside the mod file
...
PARAMETER {
    ...
    m_timeCourse_m0 = 0                    : was: 0.0 (none)
    m_steadyState_rate = 1                 : was: 1.0 (none)
    m_steadyState_midpoint = -75 (mV)      : was: -0.075 (voltage)
    m_steadyState_scale = -5.5 (mV)        : was: -0.0055 (voltage)
}
...
BREAKPOINT {
    ...
    ? DerivedVariable is based on path: conductanceScaling[*]/factor, on: Component(id=ar__m00 type=ionChannelHH), from conductanceScaling; null
    ? Path not present in component, using factor: 1
    
    conductanceScale = 1 
    
    ? DerivedVariable is based on path: gates[*]/fcond, on: Component(id=ar__m00 type=ionChannelHH), from gates; Component(id=m type=gateHHtauInf)
    ...
    ih = gion * (v - eh)
    i__ar__m00 =  -1 * ih : set this variable to the current also - note -1 as channel current convention for LEMS used!
...
PROCEDURE rates() {
    ...
    ? DerivedVariable is based on path: timeCourse/t, on: Component(id=m type=gateHHtauInf), from timeCourse; Component(id=null type=ar__m00_m_tau_tau)
